    ...
    public static String parseString(HttpResponse httpResponse) throws IOException {
		...
		if (null != entity) {
			...
			if (statusCode == 200) {
				...
				resp = "请求状态异常：" + httpResponse.getStatusLine().getStatusCode()+"\\n"+EntityUtils.toString(entity, "UTF-8");
				System.out.println("======="+resp);
				if (statusCode==302 && httpResponse.getFirstHeader("Location")!=null) {
					resp += "；Redirect地址：" + httpResponse.getFirstHeader("Location").getValue();
				}

			}
			EntityUtils.consume(entity);
		}
		System.out.println("=======http statusCode:" + httpResponse.getStatusLine().getStatusCode());
